...
function Landing({ onSignIn }) {
  return <div className="landing">
    <header className="l-header"><a className="l-brand" href="#top"><img src="/eventpay-sentinel-logo.svg" alt="" /> <span>EventPay <b>Sentinel</b></span></a><nav className="l-nav"><a href="#how-it-works">How it works</a><a href="#walkthrough">Walkthrough</a><button className="btn-primary" onClick={onSignIn}>Sign in with Google</button></nav></header>
    <main id="top">
      <section className="hero hero-new"><div className="hero-copy"><span className="hero-kicker"><i /> PAYMENT TRUST FOR LIVE EVENTS</span><h1>Every registration.<br /><em>Verified with confidence.</em></h1><p>EventPay Sentinel turns a Google Form and Razorpay into a live, evidence-backed command center for your event.</p><div className="hero-actions"><button className="btn-primary btn-lg" onClick={onSignIn}>Build your event workspace <span>→</span></button><a className="text-link" href="#how-it-works">See how it works <span>↓</span></a></div><div className="hero-proof"><span><b>01</b> One source of truth</span><span><b>02</b> Evidence before entry</span><span><b>03</b> Human-controlled decisions</span></div></div><div className="hero-visual" aria-label="EventPay Sentinel dashboard preview"><div className="visual-glow" /><div className="mini-window"><div className="mini-window-bar"><span><i /><i /><i /></span><small>EventPay Sentinel / Command center</small><b>● LIVE</b></div><div className="mini-content"><div className="mini-sidebar"><img src="/eventpay-sentinel-logo.svg" alt="" /><span className="mini-active">Overview</span><span>Registrations</span><span>Risk queue</span><span>Ask Sentinel</span></div><div className="mini-main"><div className="mini-welcome"><span>EVENT OVERVIEW</span><strong>Campus Innovators Summit</strong><small>Live payment health · Updated just now</small></div><div className="mini-metrics"><div><small>Verified</small><strong>1,248</strong><i className="up">+12.4%</i></div><div><small>Collected</small><strong>₹8.4L</strong><i className="up">+8.2%</i></div><div className="alert-metric"><small>Needs review</small><strong>07</strong><i>Action needed</i></div></div><div className="mini-panels"><div className="mini-chart"><small>PAYMENT FLOW</small><div className="chart-lines"><i /><i /><i /><i /><b /></div></div><div className="mini-alert"><small>RISK QUEUE</small><span><b>!</b> Duplicate UTR detected <em>Review</em></span><span><b>!</b> Amount mismatch <em>Review</em></span></div></div></div></div></div></div></section>
    <section className="features" id="how-it-works"><div className="section-intro"><span className="eyebrow">FROM REGISTRATION TO ENTRY</span><h2>How it works</h2><p>One clear workflow for collecting payments, catching risk, and welcoming the right people at the gate.</p></div><div className="f-grid">
       <div className="f-card"><div className="feature-top"><span className="feature-step">01</span><span className="feature-icon"><SentinelIcon type="intake" /></span></div><h3>Connect your Google Form</h3><p>Paste your form link. EventPay maps the name, phone, email, and ticket fields, then gives you a ready-to-use Apps Script.</p></div>
       <div className="f-card"><div className="feature-top"><span className="feature-step">02</span><span className="feature-icon"><SentinelIcon type="payment" /></span></div><h3>Collect registrations and payments</h3><p>Each submission gets a unique registration ID and a Razorpay payment link for the right ticket category.</p></div>
       <div className="f-card"><div className="feature-top"><span className="feature-step">03</span><span className="feature-icon"><SentinelIcon type="command" /></span></div><h3>See the truth in one place</h3><p>Track registered, paid, pending, refunded, and checked-in participants from a live event command center.</p></div>
       <div className="f-card"><div className="feature-top"><span className="feature-step">04</span><span className="feature-icon"><SentinelIcon type="risk" /></span></div><h3>Catch risky payments early</h3><p>Duplicate UTRs, amount mismatches, reused payments, and unusual activity are flagged for review before entry.</p></div>
       <div className="f-card"><div className="feature-top"><span className="feature-step">05</span><span className="feature-icon"><SentinelIcon type="ai" /></span></div><h3>Investigate with evidence</h3><p>Ask Sentinel why a payment is pending or suspicious. Get a read-only explanation linked to payment and audit records.</p></div>
       <div className="f-card"><div className="feature-top"><span className="feature-step">06</span><span className="feature-icon"><SentinelIcon type="entry" /></span></div><h3>Approve and admit safely</h3><p>Organizers make the final decision, then volunteers scan verified QR entry passes at the gate with a complete audit trail.</p></div>
    </div></section>
    <section className="proof-section"><div><span className="eyebrow">WHY SENTINEL</span><h2>Payment happened.<br /><em>Now prove who it belongs to.</em></h2></div><div className="proof-points"><div><span className="proof-number">01</span><p>Razorpay confirms the transaction.</p></div><div><span className="proof-number">02</span><p>Sentinel connects the payment to the right participant.</p></div><div><span className="proof-number">03</span><p>Your team decides who is safe to admit.</p></div></div></section>
    <section className="walkthrough-section" id="walkthrough"><div className="section-intro"><span className="eyebrow">SEE IT IN ACTION</span><h2>A five-minute tour of payment truth</h2><p>Use your walkthrough here to show judges the complete journey, from form submission to safe entry.</p></div><div className={`video-frame ${DEMO_VIDEO_ID ? "video-ready" : ""}`}>{DEMO_VIDEO_ID ? <iframe src={`https://www.youtube.com/embed/${DEMO_VIDEO_ID}`} title="EventPay Sentinel product walkthrough" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /> : <div className="video-placeholder"><span className="play-button">▶</span><strong>Your product walkthrough</strong><p>Add your YouTube video ID in <code>client/src/main.jsx</code> to embed your demo here.</p></div>}<span className="video-caption">EVENTPAY SENTINEL · PRODUCT WALKTHROUGH</span></div></section>
    </main>
    <footer className="l-footer"><div className="l-footer-brand"><img src="/eventpay-sentinel-logo.svg" alt="" /> EventPay Sentinel</div><p>Built for India's high-volume events · Razorpay-powered · Evidence before entry</p></footer>
  </div>;
}
